...
FlacFileDecoder::FlacFileDecoder(QObject *parent,QString infilename,QString outfilename)
    : QThread(parent)
{
...
FlacFileDecoder::~FlacFileDecoder()
{
    totalsamples = 0;
    samplerate = 0;
    channels = 0;
...
FLAC__bool FlacFileDecoder::write_little_endian_uint16(FILE *file, FLAC__uint16 x)
{
...
FLAC__bool FlacFileDecoder::write_little_endian_int16(FILE *file, FLAC__int16 x)
{
...
FLAC__bool FlacFileDecoder::write_little_endian_uint32(FILE *file, FLAC__uint32 x)
{
...
FLAC__StreamDecoderWriteStatus FlacFileDecoder::write_callback(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{
...
void FlacFileDecoder::metadata_callback(const FLAC__StreamDecoder *decoder, const FLAC__StreamMetadata *metadata, void *client_data)
{
...
void FlacFileDecoder::error_callback(const FLAC__StreamDecoder *decoder, FLAC__StreamDecoderErrorStatus status, void *client_data)
{
    (void)decoder, (void)client_data;
    error = FLAC__StreamDecoderErrorStatusString[status];
}

void FlacFileDecoder::Init()
{
    Objectinit();
}

void FlacFileDecoder::Objectinit()
{
    Timer = new QTimer();
    Timer->setInterval(100);
    connect(Timer,&QTimer::timeout,this,&FlacFileDecoder::Returnprogress);
    Timer->start();
}

void FlacFileDecoder::run()
{
    Decoderinit();
    Md5checkinit();
    Fileinit();
...
void FlacFileDecoder::Decoderinit()
{
...
void FlacFileDecoder::Md5checkinit()
{
    if(Decoder)
    {
...
void FlacFileDecoder::Processinit()
{
...
void FlacFileDecoder::Returnprogress()
{
