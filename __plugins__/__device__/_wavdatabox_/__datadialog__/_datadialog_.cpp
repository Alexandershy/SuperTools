...
void DataDialog::Plotinit()
{
    ...
    {
        Canvasqvector.append(new SuperPlot(this));
...
void DataDialog::Creatplotapi(SuperPlot *plot,int index)
{
...
void DataDialog::Channeltextinit(SuperPlot *plot,int channel)
{
...
{
    Threadra = new ReadWav(Wavfile,Timelength,Samplerate,Bitspersample,Bits,Channels,&Canvasqvector);
    connect(Threadra,&ReadWav::Signalra,this,&DataDialog::Signalraslot);
    connect(Threadra,&ReadWav::Signalrb,this,&DataDialog::Setprogressbar);
    connect(Threadra,&ReadWav::Signalrc,this,&DataDialog::Signalrcslot);
    connect(Threadra,&ReadWav::finished,this,&DataDialog::Reset);
    connect(Threadra,&ReadWav::finished,Threadra,&QObject::deleteLater);
    Threadra->start();
...
void DataDialog::Playinit()
{
    ...
    if(Plugin->ui->progressBar->value() == 50)
    {
        ...
        Threadpa = new PlayWav(File,Endverlineqvector.at(0)->point1->key() * Samplerate * Bitspersample,&Playstatus,audiosink);
    }
    else
    {
        Threadpa = new PlayWav(File,File->size(),&Playstatus,audiosink);
    }
    Timerta = Threadpa->Timer;
    connect(Timerta,        &QTimer::timeout,   this,       &DataDialog::Signalpcslot);
    connect(Threadpa,       &PlayWav::Signalpa, this,       &DataDialog::Signalpaslot);
    connect(Threadpa,       &PlayWav::Signalpb, this,       &DataDialog::Signalpbslot);
    connect(Threadpa,       &PlayWav::finished, Threadpa,   &QObject::deleteLater);
    Timerta->setInterval(Refreshrate);
...
{
    SuperPlot *plot = (SuperPlot*)sender();
...
{
    Threadta = new ToFft(this,Canvasqvector.at(Currentchannel),&Selection,Samplerate,Maxval,Fftcanvas,&Dbvmode);
    connect(Threadta,&ToFft::Signalta,this,     &DataDialog::Fftcanvastextapi);
    connect(Threadta,&ToFft::Signaltb,this,     &DataDialog::Setprogressbar);
    connect(Threadta,&ToFft::finished,Threadta, &QObject::deleteLater);
    Threadta->start();
    Progressbardialog->Show();
}
...
ReadWav::ReadWav(QString wavfile,int timelength,int samplerate,int bitspersample,int bits,int channels,QVector<SuperPlot*> *canvasqvector)
{
    Wavfile = wavfile;
...
void ReadWav::Objectinit()
{
    SuperM = new SuperMultiMedia(this);
}

void ReadWav::Initrun()
{
    Wavfiledata = "";
    Wavfiledatacount = 0;
    X.clear();
...
ToFft::ToFft(QWidget *parent,SuperPlot *timeplot,QCPDataSelection *selection,int samplerate,int maxval,SuperPlot *fftplot,QByteArray *dbvmode)
    : QThread(parent)
{
    Timeplot = timeplot;
    Selection = selection;
...
void ToFft::Calrealdata(int count,fftw_complex* complex,double* yreal,QVector<double>* y)
{
    double temp = 0;
    for (int i = 0; i < count / 2; i++)
    {
...
void ToFft::Fftcanvasinfo(double *ylffti,double *ylfftro,int count)
{
